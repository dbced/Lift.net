...
Public Class FrmImpianto
    ...
    Private Sub FrmImpianto_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            'Dim el As ImagePrimitive = New ImagePrimitive()
            'el.Alignment = ContentAlignment.MiddleLeft
            'el.Image = ImageList24.Images(2)
            'Dim strip = CType(ToolWinSchedaTec.TabStrip, ToolTabStrip)
            'strip.CaptionElement.Children(2).Children.Add(el)

            RPcontainer.Pages.Item(4).Visible = False

            Dim elm As ImagePrimitive = New ImagePrimitive()
            elm.Image = ImageList24.Images(5)
            Dim stripM = CType(ToolWinMap.TabStrip, ToolTabStrip)
            stripM.CaptionElement.Children(2).Children.Add(elm)

            Dim elg As ImagePrimitive = New ImagePrimitive()
            elg.Image = ImageList24.Images(4)
            Dim stripG = CType(ToolWinGrafico.TabStrip, ToolTabStrip)
            stripG.CaptionElement.Children(2).Children.Add(elg)
    ...
    Private Async Sub async_carica_griglia_DatiTecnici_asset_impianto(IdAsset As String, CodImp As String)
        Try
            If formInCaricamento = False Then
                wb.AssociatedControl = gridTecAsset
                wb.StartWaiting()
                wb.Visible = True
            End If
    ...
    Private Async Sub async_carica_grafico_fatturato(codImp As String, anno As String, Optional livello As Integer = 0)
        Try
            If formInCaricamento = False Then
                wb.AssociatedControl = chartFatturato
                wb.StartWaiting()
                wb.Visible = True
            End If
    ...
    Private Async Sub async_carica_griglia_documenti(codImp As String, anno As String, cliente As String)
        Try

            Dim elementi As Threading.Tasks.Task(Of List(Of elencoDocumenti))
            elementi = ws.getDocumentiImpianto(codImp, anno, cliente)
            Await elementi
    ...
    Private Function controlli_form() As Boolean
        ...
        Try
            ...
            If cmbCategoriaImp.SelectedIndex = -1 Then
                Telerik.WinControls.RadMessageBox.Show(Me, "Selezionare la catecoria impianto", "Anagrafica impianti", MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                cmbCategoriaImp.Focus()
                Exit Function
            End If
...
#End Region
    ...
    Private Sub gridDoc_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles gridDoc.CellDoubleClick
        Try
            ...
            'sUrl = "http://100.100.100.26/EditDocument.aspx?DOC=31472&BD=012"
            'Process.Start(sUrl)
            ...
            Dim id As String = cl.retrive_documento_EDOC(dbEdoc, "TipoDocumento=014&ND=" & ndoc & "&DD=" & dtdoc)
            If id <> "" Then
                sUrl = "http://100.100.100.26/EditDocument.aspx?DOC=" & id & "&BD=" & dbEdoc
                Process.Start(sUrl)
            End If
    ...
    Private Sub ToolWinGrafico_LostFocus(sender As Object, e As EventArgs) Handles ToolWinGrafico.LostFocus
        Try
            'Dim elg As ImagePrimitive = New ImagePrimitive()
            'elg.Image = ImageList24.Images(4)
            'Dim stripG = CType(ToolWinGrafico.TabStrip, ToolTabStrip)
    ...
    Private Sub ToolWinGrafico_GotFocus(sender As Object, e As EventArgs) Handles ToolWinGrafico.GotFocus
        Try
            'Dim elg As ImagePrimitive = New ImagePrimitive()
            'elg.Image = ImageList24.Images(3)
            'Dim stripG = CType(ToolWinGrafico.TabStrip, ToolTabStrip)
            'stripG.CaptionElement.Children(2).Children.Add(elg)
        Catch ex As Exception
